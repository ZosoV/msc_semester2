...
The week 01 topics are:

1. Regular Expressions
   - Basic ELIZA Understanding
2. Words and Corpora
3. Text-Normalization
   - Tokenization (Byte-Pair Coding)
...
A regular expression search function (in Python or Unix) will search through the corpus of texts, returning all texts that match the pattern.

- Notice the examples in the following overview only show the first match.
- Notice what we understand by **string**. In general, you have to see a string as any sequence of characters, and these characters can be alphanumeric, underscore, special chars, and even blank spaces. This is the general view when we talk about strings.

The following regex rules were covered:
...
4. **Optional, Zero or More, One or More, Any Character**: 
   - The symbols `?`, `*`, `+`, and `.` are used for optional characters matching:
      - `?`: one optional char
...
5. **Anchors**: 
   ...
   - A **word** for a regular expression point of view is defined as any sequence of digits, underscores, or letters.
...
8. **Lookahead Assertions**: 
   ...
   - Example 2: The regex `q(?=u)` will match the letter 'q' only if it is immediately followed by the letter 'u'. In the string "quiet", it matches the 'q', but does not include the 'u' in the match. So, the overall match is just 'q'. Notice, that it will not match the 'q' in the word 'Iraq' because it's not inmediately followed by a letter 'u'.
...
| Non-Capture Group | `(?:some\|a few) (people\|cats) don't like other \1` | some cats don't like other cats |
